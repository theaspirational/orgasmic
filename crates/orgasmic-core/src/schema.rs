...
// orgasmic:dec_WWAHT,term_5N4M6
/// Per-node layout entry from `graph_layout.org` (overlay only, never topology).
...
/// Parse the `graph_layout.org` overlay into a node-id → entry map.
/// Iterates top-level headings; headings without `:ID:` are skipped.
/// Missing file callers should pass an empty `OrgFile`; missing properties
/// are represented as `None`.
pub fn parse_graph_layout_overlay(
    file: &OrgFile,
) -> std::collections::HashMap<String, GraphLayoutEntry> {
    ...
    for heading in &file.headings {
        ...
        let entry = GraphLayoutEntry {
            x: heading.property("X").and_then(|v| v.trim().parse().ok()),
            y: heading.property("Y").and_then(|v| v.trim().parse().ok()),
            w: heading.property("W").and_then(|v| v.trim().parse().ok()),
            h: heading.property("H").and_then(|v| v.trim().parse().ok()),
            hidden: heading
                .property("HIDDEN")
