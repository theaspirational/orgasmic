...
//! plan, reconcile, forum). TASK-005 promotes `serve`, `status`,
...
Examples:
  ...
  orgasmic path print                  # print the line to add yourself")]
    ...
    /// Run multi-model deliberation modes.
    Forum(forum::ForumArgs),
...
enum TaskCmd {
    ...
    Update {
        ...
        /// keyword and the org tags on the same line are preserved; tags are
        /// a separate concern and this flag never writes them. Runs as its
        /// own edit, so it cannot be combined with the fields below.
        #[arg(long)]
        title: Option<String>,
        /// Optimistic-concurrency token for `--title`, as reported by
    ...
    runtime.block_on(async move {
        ...
        let value: serde_json::Value = match cmd {
            ...
            TaskCmd::Create {
                ...
                let request_id =
                    resolve_create_request_id(request_id, "task.create", &project, &body);
            ...
            TaskCmd::Update {
                ...
                // orgasmic:task_XPYRR
                // A heading-line write, not a drawer write: it goes through the
                // org-node editor for OCC + the structural guard. Combining it
                // with the drawer/lifecycle fields would mean two writes and two
                // txs behind one command, so it is refused rather than half-done
                // — and `--reason` is refused with them rather than dropped.
                if state.is_some()
                    || priority.is_some()
                    || reason.is_some()
                    || !properties.is_empty()
                {
                    anyhow::bail!(
                        "--title rewrites the heading line through the org-node editor and cannot \
                         share a call with --state/--priority/--property/--reason; run it as its \
                         own `orgasmic task update <id> --title \"...\"`"
                    );
                }
                let project = manager::resolve_project(project)?;
                node::set_node_title(
                    ...
                    node::NodeTitleWrite {
                        ...
                        kind: Some(orgasmic_core::NodeKind::Task.as_str()),
                        title: &title,
                        base_version,
                        request_id,
                        json,
    ...
    runtime.block_on(async move {
        ...
        let value: serde_json::Value = match cmd {
            ...
            GlossaryCmd::Create {
                ...
                let request_id =
                    resolve_create_request_id(request_id, "glossary.create", &project, &body);
                body["request_id"] = serde_json::json!(request_id);
    ...
    runtime.block_on(async move {
        ...
        match cmd {
            ...
            DecisionCmd::Create {
                ...
                let request_id =
                    resolve_create_request_id(request_id, "decision.create", &project, &body);
                body["request_id"] = serde_json::json!(request_id);
...
/// Derive a stable default `--request-id` from the operation, resolved project,
/// and canonical create payload when the caller omits one. Explicit ids win.
fn resolve_create_request_id(
    ...
    Some(default_create_request_id(operation, project, payload))
}

fn default_create_request_id(
    operation: &str,
    project: &str,
    payload: &serde_json::Value,
) -> String {
...
mod create_request_id_tests {
    ...
    fn default_request_id_stable_for_identical_payload() {
        ...
        assert_eq!(
            default_create_request_id("glossary.create", "orgasmic", &a),
            default_create_request_id("glossary.create", "orgasmic", &b)
        );
    }
    ...
    fn explicit_request_id_wins() {
        ...
        assert_eq!(
            resolve_create_request_id(
                Some("explicit-1".into()),
                "task.create",
                "orgasmic",
                &payload
            )
