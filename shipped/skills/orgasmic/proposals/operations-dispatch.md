...
proposal:
  action: update
  target: operations/dispatch
  note: TASK-CS2TM adds manager dispatch-status --json; source is current CLI help
    from crates/orgasmic-cli/src/manager.rs.
  created: '2026-09-02'
---
...
`manager dispatch-status --json` emits one object with separate torn-close,
cleanup-failed, open-dispatch, and managed-worktree branches.
