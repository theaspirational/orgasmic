...
1. A first line containing exactly `__ORGASMIC_RUN_STATS__`. The orchestrator
   replaces it with the deterministic run-stats block (participant roster,
   curator identity, report counts, start time). Never write your own roster
   or run header.
