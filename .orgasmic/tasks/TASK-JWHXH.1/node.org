...
* IN_PROGRESS TASK-JWHXH.1 H4+M1: views/ tracked on existing ledgers (only the scaffold ignores it) and never regenerated after incremental writes :e01mc:chain-review:
:PROPERTIES:
:ID: TASK-JWHXH.1
:PRIORITY: P1
:FIX_SUBTASK: t
...
** Notes
RELEASE NOTE (review finding 3, MEDIUM, unverified on hardware): mixed-version fleet hazard. A machine still on a pre-c3d779af daemon commits MODIFICATIONS to tracked views/*.org while a fixed machine commits their DELETION; the old machine's pull --rebase hits a modify/delete conflict and its ledger sync stalls every tick until upgraded. Upgrade every machine before the first fixed daemon pushes the untrack. On this project the live ledger was hand-cut-over 2026-09-01 so the fixed code is a no-op here; gigabyte has no GitHub write creds (its push already fails), so exposure there is nil in practice, but its runtime version was not checked from this session.
