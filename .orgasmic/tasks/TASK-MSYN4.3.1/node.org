...
* DONE TASK-MSYN4.3.1 Test-only: pin the fold contract for one tx id shared by two machine generations :e01mc:chain-review:
:PROPERTIES:
:ID: TASK-MSYN4.3.1
:PRIORITY: P3
...
** Evidence
Merged ad8c104d (worker 753136c0, codex gpt-5.6-sol): test-only +36 in crates/orgasmic-core/src/tx.rs — two machines start generations sharing legacy id tx-20260901-orgasmic-0007, one CLOSED_TX; pins the existing fold contract (the close selects the latest matching start; the earlier duplicate stays visibly open). Optional ProjectSequence rename skipped. Manager gates re-run on merged main 2026-09-01: cargo test -p orgasmic-core --lib tx → 26 passed; clippy core -D warnings clean; fmt clean. Logs: scratchpad/gates-MSYN4.3.1-*.log.
