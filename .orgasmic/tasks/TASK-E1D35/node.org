...
* DONE TASK-E1D35 Forum review rounds: 'forum review' challenges existing reports with a chosen panel :feature:
:PROPERTIES:
...
** Evidence
- Implementer: codex/gpt-5.6-sol/xhigh; follow-ups 0e6f816c; merged at f044ba19
- Review: claude-fable-5/high, APPROVE 'merge as-is'; MEDIUM: self-exclusion included harness so the same model via another transport saw its own report — fixed to vendor+model identity with a cross-harness test; LOW casefold/trim of vendor-model at parse; LOW review rounds get a 300px diagram slot (no blank band)
- New prompt spec forum-reviewer.org (cross-reviewer reuse judged genuinely unfit by reviewer)
- Gates: 299 CLI bin tests, clippy -D warnings, fmt, ask SVG fixture byte-identical, both reviewer mutation probes red-then-green
- No live billed review-round smoke yet (operator-owned)
