...
* IN_PROGRESS TASK-8DWJP.1.3 Narrow fix round for 8DWJP.1.2: salvage the worktree before every rebase --abort :e01mc:chain-review:
:PROPERTIES:
:ID: TASK-8DWJP.1.3
:PRIORITY: P1
...
** Evidence
Merged 1ff48c3a (worker 5846e9bb, codex gpt-5.6-sol): one abort_rebase_with_salvage helper at both rebase --abort sites (entry path and in-tick), base = rebase-merge/orig-head or rebase-apply/orig-head with ORIG_HEAD fallback; salvage ref carried via ConflictSource::Worktree into status/event; unreachable entry-path rebase branch removed; status wording 'raw worktree snapshot at <ref> (conflicted paths carry markers)'; conflicting_two_writer_tick's tracked write moved before its conflict tick; new interrupted-rebase regression writes a tracked task node + the machine tx file during the outage and reads both back from the status-named salvage ref (red before the change, green after — implementer logs cited). Manager gates re-run on merged main 2026-09-01: cargo test -p orgasmic-daemon --lib -- ledger_sync status sync_conflict barrier → 30 passed; cargo test -p orgasmic-cli --bin orgasmic -- daemon_lifecycle → 22 passed; clippy daemon+cli -D warnings clean; fmt clean. Logs: scratchpad/gates-8DWJP.1.3-*.log. Manager pre-check: unmerged paths are read before the abort (ledger_sync.rs:106); an interrupted rebase with an EMPTY unmerged set mints a salvage ref that is not reported (bounded; reviewer asked to size). Review round (claude-opus-5 high) dispatched.
