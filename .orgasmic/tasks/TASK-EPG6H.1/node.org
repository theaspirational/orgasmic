...
* IN_PROGRESS TASK-EPG6H.1 H3: torn-close repair arms read tx/ and machines/*/tx only, but task.state_transitioned now lives in journals — the operator-intent guard is dead :e01mc:chain-review:
:PROPERTIES:
:ID: TASK-EPG6H.1
:PRIORITY: P1
:FIX_SUBTASK: t
...
** Evidence
Merged beee263b (worker 1139bc0f, codex gpt-5.6-sol). Manager re-ran gates on merged main 2026-09-01: cargo test -p orgasmic-cli --bin orgasmic -- torn_close → 1 passed; cargo test -p orgasmic-daemon --lib -- repair → 5 passed; -- evidence → 6 passed (incl. recorded_close_repair_yields_to_later_journal_transition and the extended task_close_requires_a_nonempty_evidence_section); cargo clippy -p orgasmic-cli -p orgasmic-daemon --all-targets -D warnings clean; cargo fmt --all --check clean. Note: orgasmic-cli has no lib target; the --lib gate in the brief maps to --bin orgasmic with a filter. Review round: claude-opus-5 high over beee263b^1..beee263b (verdict on the reviewer close).
