...
* DONE TASK-KA934.1.1 Fix round 2 for KA934.1: OCC conflict → 409, surface edit/delete stamps and tombstones in activity, pin the two test gaps :e01mc:chain-review:
:PROPERTIES:
:ID: TASK-KA934.1.1
:PRIORITY: P2
...
** Evidence
Merged 8cff0347 (worker 8967bd4e, codex gpt-5.6-sol): typed path-free comment OCC/not-found errors mapped to 409/404 (403 authorship kept); activity projects EDITED_BY/EDITED_AT/DELETED_BY/DELETED_AT and keeps comment.deleted rows as empty-body tombstones so reply anchors never dangle; UI renders one-line tombstones without Reply/Edit/Delete and marks edited comments; tests pin stale-edit 409 with unchanged journal bytes, exact authorship refusal text, activity tombstone stamps, sequential-edit stamp replacement (EDITED_BY once, second editor), tombstone UI actions. Manager gates re-run on merged main 2026-09-01: cargo test -p orgasmic-daemon --lib -- comment activity → 22 passed; cargo test -p orgasmic-core --lib node_kernel → 3 passed; clippy core+daemon -D warnings clean; fmt clean; cd ui && npm run typecheck clean. Logs: scratchpad/gates-KA934.1.1-*.log. Review round waived: LOW-origin second round, additive, fully pinned by tests.
